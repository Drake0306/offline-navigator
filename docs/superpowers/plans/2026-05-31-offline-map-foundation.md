...
> - **`maplibre` types (CORRECTED after reading the installed 0.3.5 package during Task 10 — supersedes earlier guesses):** widget `MapLibreMap({MapOptions? options, void Function(MapController)? onMapCreated, void Function(MapController)? onStyleLoaded, List<Source> sources, List<Layer> layers, ...})`. **Both `onMapCreated` and `onStyleLoaded` deliver a `MapController`** — there is NO separate `StyleController` passed to callbacks. The style is supplied via `MapOptions(style: <url-or-json>, center: Position(lng,lat), initZoom:, initBearing:, initPitch:)`. **Camera coordinates use `Position(lng, lat)` from the `geotypes` package (re-exported by maplibre) — NOT `Geographic`.** Controller camera control: `MapController.animateCamera({Position? center, double? zoom, double? bearing, double? pitch, Duration nativeDuration})` and `moveCamera({...})`; `getCamera()` returns a `Future<MapCamera>` (async). For sources/layers (Task 11): `MapLibreMap` accepts declarative `sources:`/`layers:` lists, OR add imperatively via the `MapController` (confirm exact `addSource`/`addLayer`/`addImage`/`updateGeoJsonSource` names against the installed package — they may live on a style-manager object exposed by the controller). The built-in `enableLocation()`/`trackLocation()` location component is also available as a pointer fallback.
